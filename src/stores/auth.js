...
export const useAuthStore = defineStore("auth", () => {
  ...
  const validateToken = async () => {
    try {
      const response = await axios.get(
        `${API_END_POINT}/courtmaster/auth/validate-token`
      );
  ...
  const getBookingScheduleHitories = async () => {
    try {
        ...
        const response = await axios.post(`${API_END_POINT}/courtmaster/filter/history/booking-schedules`, payload.value);
...
*/
  ...
  const checkTokenValidity = async () => {
    // Hàm này chỉ được gọi khi đã có thông tin trong local Storage.
