...
const axiosInstance = axios.create({
  baseURL: `${API_END_POINT}`, // Đặt baseURL của bạn ở đây
...
axiosInstance.interceptors.response.use(
  // interceptors này hoạt động như một bộ xử lý lỗi toàn cục cho tất cả các response nhận được bởi thể hiện Axios này.
  (response) => response, // Chỗ này xử lý các response thành công.
  (error) => {
    // nếu request thất bại thì sẽ được xử lý ở chỗ này. (lỗi mạng, timeout hoặc mã trạng thái không phải 2xx).
    if (error.response) {
      // Kiểm tra xem lỗi có thuộc tính response không. Điều này cho biết server đã phản hồi với một trạng thái lỗi.
